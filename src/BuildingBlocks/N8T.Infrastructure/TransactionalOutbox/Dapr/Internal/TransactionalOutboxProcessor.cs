...
namespace N8T.Infrastructure.TransactionalOutbox.Dapr.Internal
{
    internal class TransactionalOutboxProcessor : ITransactionalOutboxProcessor
    {
        ...
        {
            _logger.LogInformation("{TransactionalOutboxProcessor}: Cron @{DateTime}", nameof(TransactionalOutboxProcessor), DateTime.UtcNow);
            ...
            if (events?.Value == null || !(events?.Value.Count > 0)) return;
